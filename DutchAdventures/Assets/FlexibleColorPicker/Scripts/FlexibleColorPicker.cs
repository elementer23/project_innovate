...
public class FlexibleColorPicker : MonoBehaviour {
    ...
    */
    ...
    public CharacterCustHandler charCustHandler;
    ...
    */
    ...
    private BufferedColor PickColor1D(BufferedColor color, PickerType type, float value) {
        switch(type) {
        case PickerType.R: return color.PickR(Mathf.Lerp(avs.r.min, avs.r.max, value));
        case PickerType.G: return color.PickG(Mathf.Lerp(avs.g.min, avs.g.max, value));
        case PickerType.B: return color.PickB(Mathf.Lerp(avs.b.min, avs.b.max, value));
        case PickerType.H: return color.PickH(Mathf.Lerp(avs.h.min, avs.h.max, value) * HUE_LOOP); 
        case PickerType.S: return color.PickS(Mathf.Lerp(avs.s.min, avs.s.max, value));
        case PickerType.V: return color.PickV(Mathf.Lerp(avs.v.min, avs.v.max, value));
        case PickerType.A: return color.PickA(Mathf.Lerp(avs.a.min, avs.a.max, value));
        default:
    ...
    */
    ...
    private float GetValue1D(PickerType type) {
        switch(type) {
        case PickerType.R: return Mathf.InverseLerp(avs.r.min, avs.r.max, bufferedColor.r);
        case PickerType.G: return Mathf.InverseLerp(avs.g.min, avs.g.max, bufferedColor.g);
        case PickerType.B: return Mathf.InverseLerp(avs.b.min, avs.b.max, bufferedColor.b);
        case PickerType.H: return Mathf.InverseLerp(avs.h.min, avs.h.max, bufferedColor.h / HUE_LOOP);
        case PickerType.S: return Mathf.InverseLerp(avs.s.min, avs.s.max, bufferedColor.s);
        case PickerType.V: return Mathf.InverseLerp(avs.v.min, avs.v.max, bufferedColor.v);
        case PickerType.A: return Mathf.InverseLerp(avs.a.min, avs.a.max, bufferedColor.a);
        default:
            throw new Exception("Picker type " + type + " is not associated with a single color value.");
        }
    }
    ...
    */
    ...
    private void UpdateTextures() {
        foreach(PickerType type in Enum.GetValues(typeof(PickerType))) {
            if(staticMode || avs.Get((int)type).overrideStatic)
                UpdateStatic(type);
            else
                UpdateDynamic(type);
        }
    ...
    private void UpdateDynamic(PickerType type) {
        ...
        m.SetVector(SHADER_HSV_MIN, new Vector4(avs.h.min, avs.s.min, avs.v.min));
        m.SetVector(SHADER_HSV_MAX, new Vector4(avs.h.max, avs.s.max, avs.v.max));
    }
